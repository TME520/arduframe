...
At startup, the sketch draws one second of red/green/blue/white color bars before initializing the SD card. If those bars remain white or invisible, the failure is in the TFT controller/pin initialization rather than BMP decoding or SD reading. The startup log prints the selected controller and drawable size so you can confirm the build. If an auto-detected MCUFRIEND ID, such as `0x7575`, initializes with an unusable drawable size like `320x0`, the sketch now retries a short list of known MCUFRIEND-compatible controller IDs and uses the first one that reports a non-zero width and height. If every retry still reports a zero dimension, set `ARDUFRAME_FORCE_TFT_ID` in `arduframe.ino` to the matching shield controller and upload again.
